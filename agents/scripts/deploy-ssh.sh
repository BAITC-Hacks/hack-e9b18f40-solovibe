...
[[ -f "$root/upload-$revision.tar" && -f "$root/runtime.env" ]] || { echo "Provide the release archive and runtime.env first" >&2; exit 1; }
...
tar -xf "$root/upload-$revision.tar" -C "$release"
rm "$root/upload-$revision.tar"
compose() { docker compose --project-name solovibe-production --env-file "$root/runtime.env" -f "$release/compose.production.yaml" "$@" </dev/null; }
