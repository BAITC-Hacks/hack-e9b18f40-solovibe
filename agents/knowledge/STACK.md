...
The Docker image contains the standalone app and migration runner. Delivery is local verification → commit/push → manual SSH deployment of that exact revision → server build → migrate → replace app → check revision. The checkpoint command runs manual deployment after its push; after a standalone push, run node agents/scripts/manual-deploy.mjs. This command updates the local Docker application and the SSH server concurrently and waits for both health checks. Production Compose keeps persistent PostgreSQL and a loopback app port. The public Brev Secure Link terminates HTTPS; Nginx passes streaming responses without buffering. Read agents/ops/SETUP.md for the provisioning and release procedure.
