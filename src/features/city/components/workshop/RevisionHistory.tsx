...
import { EvaluationSummary, SnapshotLabel } from "./Comparison";
...
export function RevisionHistory({ scenarioId, activeRevisionId, selected, onSelect, onApply, onFork, busy }: Props) {
    ...
    useEffect(() => { const a = new AbortController(); queueMicrotask(() => { if (!a.signal.aborted) {
        setLoading(true);
        setProblem(null);
    } }); cityApi<HistoryPage>(`/scenarios/${scenarioId}/history${cursor ? `?cursor=${encodeURIComponent(cursor)}` : ""}`, { signal: a.signal }).then(next => { if (!a.signal.aborted)
        setPage(prior => ({ items: cursor ? [...prior.items.filter(s => !next.items.some(n => n.revision.id === s.revision.id)), ...next.items] : next.items, nextCursor: next.nextCursor })); }).catch(e => { if (!a.signal.aborted) {
        const code = errorCode(e);
        setProblem(te.has(code) ? te(code) : t("requestFailed"));
    } }).finally(() => { if (!a.signal.aborted)
        setLoading(false); }); return () => a.abort(); }, [scenarioId, activeRevisionId, cursor, retry, t, te]);
    async function saveTitle(id: string) { if (!title.trim())
        return; setSaving(true); setProblem(null); const a = new AbortController(); mutations.current.add(a); try {
