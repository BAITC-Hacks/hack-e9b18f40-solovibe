...
export function StressPanel() {
 const router=useRouter(),tw=useTranslations("workflow");
    ...
    const request = useCallback(async <T,>(path: string, method = "GET", body?: unknown) => { const a = new AbortController(); requests.current.add(a); try {
        return await cityApi<T>(path, { method, body, signal: a.signal });
    }
    finally {
        requests.current.delete(a);
    } }, []);
    const report = useCallback((error: unknown) => { if (!mounted.current)
        return; const code = errorCode(error); setProblem(te.has(code) ? te(code) : t("requestFailed")); }, [te, t]);
    const idFor = (key: string) => { let id = tokens.current.get(key); if (!id) {
        id = crypto.randomUUID();
        tokens.current.set(key, id);
    } return id; };
    const scenarioId = c.serverView.scenario.id, revisionId = c.serverView.revision.id;
    useEffect(() => { const a = new AbortController(); requestList(); async function requestList() { try {
        ...
        if (!a.signal.aborted)
            report(error);
    } } return () => a.abort(); }, [scenarioId, revisionId, reload, report]);
    useEffect(() => { if (!chosen)
        return; const a = new AbortController(); queueMicrotask(() => { if (!a.signal.aborted)
        setLoading(true); }); cityApi<StressView>(`/stress-tests/${chosen}`, { signal: a.signal }).then(result => { if (!a.signal.aborted)
        setView(result); }).catch(error => { if (!a.signal.aborted)
        report(error); }).finally(() => { if (!a.signal.aborted)
        setLoading(false); }); return () => a.abort(); }, [chosen, revisionId, reload, report]);
    ...
    async function act(action: () => Promise<void>) { if (mutating.current)
        return; mutating.current = true; setBusy(true); setProblem(null); setNotice(null); try {
        await action();
    }
    catch (error) {
        report(error);
    ...
    async function create() { await act(async () => { if (!await latest.current.flush()) {
        setProblem(t("saveFirst"));
        return;
    } if (!mounted.current)
        return; const saved = await request<ScenarioView>(`/scenarios/${scenarioId}`); if (!mounted.current)
        return; if (latest.current.isDirty || draftFingerprint(saved.revision.decisions, saved.revision.constraints) !== draftFingerprint(latest.current.decisions, latest.current.constraints)) {
        setProblem(t("saveFirst"));
        return;
    } const key = JSON.stringify([saved.revision.id, selectedMeasure, cost, delay]); const result = await request<StressView>("/stress-tests", "POST", { sourceRevisionId: saved.revision.id, measureId: selectedMeasure, costIncreasePct: cost, extraLagQuarters: delay, clientMutationId: idFor(key) }); if (!mounted.current)
        return; tokens.current.delete(key); setChosen(result.experiment.id); setView(result); setJob(null); setRun(null); setReload(v => v + 1); }); }
    async function makeBrief(revisionId:string){if(!view)return;await act(async()=>{if(!await latest.current.flush())return;const brief=await request<BriefView>('/briefs','POST',{sourceRevisionId:revisionId,stressId:view.experiment.id,locale,clientMutationId:crypto.randomUUID()});if(mounted.current)router.push(`/city/${brief.brief.scenarioId}/brief?briefId=${brief.brief.id}`);});}
    async function repair(ai: boolean) { if (!view)
        return; const experiment = view.experiment; await act(async () => { const key = `${experiment.id}:${ai ? objective || t("defaultObjective") : "exact"}`; if (ai) {
        ...
        if (mounted.current)
            setJob(result);
    } tokens.current.delete(key); if (mounted.current) {
        setPaused(false);
        setReload(v => v + 1);
    } }); }
    async function cancel() { await act(async () => { if (jobId) {
        ...
        setReload(v => v + 1); }); }
    async function apply(candidateId: string, fork = false) { await act(async () => { if (fork) {
        ...
        return; const saved = await request<ScenarioView>(`/scenarios/${scenarioId}`); if (!mounted.current)
        return; if (saved.revision.id !== latest.current.serverView.revision.id) {
        ...
        setProblem(t("saveFirst"));
        return;
    } const before = fingerprint(latest.current); try {
 ...
 {history.length > 0 && <label className="cbs-history">{t("history")}<select disabled={active || busy} value={chosen ?? ""} onChange={e => { setChosen(e.target.value); setView(null); setJob(null); setRun(null); }}>{history.map(item => <option key={item.id} value={item.id}>{new Intl.DateTimeFormat(locale, { dateStyle: "short", timeStyle: "short" }).format(new Date(item.createdAt))} · {tm(item.assumption.measureId)} · +{item.assumption.costIncreasePct}% · +{item.assumption.extraLagQuarters} {t("quarters")}</option>)}</select></label>}
 {loading && <p role="status">{t("loading")}</p>}
 {experiment && <div className="cbs-result"><div className="cbs-source"><h4>{tm(experiment.assumption.measureId)} · +{experiment.assumption.costIncreasePct}% · +{experiment.assumption.extraLagQuarters} {t("quarters")}</h4><p>{t("source")}: {experiment.sourceRevisionId.slice(0, 8)}</p></div>{stale && <p className="cbs-warning">{t("stale")}</p>}
 <div className="cbs-columns"><article className="cbs-evaluation"><h4>{t("baseline")}</h4><EvaluationSummary result={experiment.baseline}/></article><article className={`cbs-evaluation ${experiment.stressed.valid ? "" : "cbs-invalid"}`}><h4>{t("experiment")}</h4><EvaluationSummary result={experiment.stressed}/>{!experiment.stressed.valid && <p className="cbs-warning">{t("invalid")}</p>}{experiment.stressed.issues.map((issue, i) => <p key={i}>{domain.has(issue.code) ? domain(issue.code, Object.fromEntries(Object.entries(issue.params).map(([k, v]) => [k, typeof v === "boolean" ? String(v) : v]))) : t("validationIssue", { code: issue.code })}</p>)}</article></div>
 <details><summary>{t("indicatorChanges")}</summary><div className="cbs-districts">{experiment.stressed.districts.map(d => { const original = experiment.baseline.districts.find(b => b.districtId === d.districtId); return <article key={d.districtId}><h5>{td(d.districtId)}</h5><dl>{d.indicators.map(i => { const from = original?.indicators.find(b => b.indicatorId === i.indicatorId)?.after; return <div key={i.indicatorId}><dt>{ti(i.indicatorId)}</dt><dd>{from === undefined ? "—" : number(from)} → {number(i.after)}</dd></div>; })}</dl></article>; })}</div></details>
 <section className="cbs-repair"><h4>{t("repairTitle")}</h4><p>{t("repairHint")}</p><div className="cbs-actions"><Button disabled={busy || active || loading} onClick={() => void repair(false)}>{t("repairExact")}</Button><Button variant="secondary" disabled={busy||active} onClick={()=>void makeBrief(experiment.sourceRevisionId)}>{tw("createBrief")}</Button>{active && <Button variant="secondary" disabled={busy} onClick={() => void cancel()}>{t("cancel")}</Button>}</div><details><summary>{t("assistant")}</summary><label>{t("objective")}<textarea rows={3} maxLength={2000} value={objective} onChange={e => setObjective(e.target.value)} placeholder={t("defaultObjective")}/></label><Button variant="secondary" disabled={busy || active || loading} onClick={() => void repair(true)}>{t("repairAi")}</Button></details>
 ...
 <div className="cbs-repairs">{view?.repairs.map((repair, i) => <article className="cbs-candidate" key={repair.revision.id}><h4>{repair.revision.title || `${t("candidate")} ${i + 1}`}</h4><h5>{t("sameAssumption")}</h5><EvaluationSummary result={repair.sensitivity}/><ConditionChecks constraints={repair.revision.constraints} result={repair.sensitivity}/>{!repair.sensitivity.valid && <p className="cbs-warning">{t("invalid")}</p>}{!repair.revision.decisions.some(d => d.measureId === experiment.assumption.measureId) && <p className="cbs-warning">{t("riskRemoved", { measure: tm(experiment.assumption.measureId) })}</p>}<details><summary>{t("decisions")}</summary><DecisionChanges source={experiment.baseline.decisions} target={repair.revision.decisions}/><h5>{t("official")}</h5><EvaluationSummary result={repair.evaluation.result}/></details><p className="cbs-hint">{t("applyHint")}</p><div className="cbs-actions"><Button disabled={busy || !!c.conflict || !repair.sensitivity.valid || loading} onClick={() => void apply(repair.revision.id)}>{t("apply")}</Button><Button variant="secondary" disabled={busy || loading} onClick={() => void apply(repair.revision.id, true)}>{t("fork")}</Button><Button variant="quiet" disabled={busy} onClick={()=>void makeBrief(repair.revision.id)}>{tw("createBrief")}</Button></div></article>)}</div></section></div>}
 </section>;
}
