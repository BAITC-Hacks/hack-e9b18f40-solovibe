...
export async function createArtifact(p: Principal, input: z.infer<typeof createArtifactSchema>) {
    ...
    const cacheKey = createHash('sha256').update(JSON.stringify(['public-safe-v1',input.revisionId, brief?.brief.id ?? null, brief?.brief.version ?? null, input.locale, input.kind])).digest('hex');
